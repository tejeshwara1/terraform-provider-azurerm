	...
	response.Schema = schema.Schema{
		Attributes: map[string]schema.Attribute{
			...
			"wait_for_completion": schema.BoolAttribute{
				Optional:    true,
				Description: "Whether to poll the job execution for completion and report its status. Defaults to `false`",
			},

			// TODO: action wrapper interface so we can handle custom timeouts in a consistent manner?
			// e.g. -> actionWithCustomTimeout?
			"timeout": schema.StringAttribute{
				Optional:    true,
				Description: "Timeout duration for the job execution. Defaults to `15m`.",
			},
		},
	}
}
...
func (m *MsSqlJobExecuteAction) Invoke(ctx context.Context, request action.InvokeRequest, response *action.InvokeResponse) {
	...
	if model.WaitForCompletion.ValueBool() {
		response.SendProgress(action.InvokeProgressEvent{
			Message: fmt.Sprintf("waiting for execution completion on %s", id.ID()),
		})
